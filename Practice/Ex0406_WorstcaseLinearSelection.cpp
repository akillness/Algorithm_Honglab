...
//    for (int i=lo; i < g ; i++){
//        int idx = -1;
//        for(int j=i; j<=hi ;j+=g){
//            if(arr[j] < arr[i+2*g]){ // 중간값을 기준값으로 설정함
//                idx++;
////                cout <<"idx : "<< idx <<", swap(" << arr[j]  << ", " << arr[i+idx*g] << ")"<<endl;
//                swap(arr[j], arr[i+idx*g]);  // 기준 값보다 작을 경우, 기준점보다 앞 인덱스로만 swap
//            }
//        }
//    }
    
    for (int i=lo ; i< hi; i++){
        SelectionSortPass(arr, i, hi, g); // 배열 전체값에 대해 stride 내에서 정렬
    }
    // 그룹단위 정렬 후 출력
    PrintGroups(arr, lo, g);
    
    // 가운데 몰려있는 그룹별 중간값들에 대해 재귀 호출
    Pair x = Select(arr, lo+2*g, lo+3*g-1, std::ceil(g/2.0)); // 중간값의 중간값 찾기

     cout << "lo = " << lo << ", hi = " << hi << ", Median of medians = " << x.value << endl;

//     중간값들의 중간값을 피벗으로 사용
     swap(arr[x.index], arr[hi]); // 배열 맨 끝으로 이동
